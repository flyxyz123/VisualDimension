...
void find_show_contours(CvMemStorage* storage, CvSeq** first_contour)
{
    ...
    cvZero(img_black); //全黑
    img2=cvLoadImage(name,CV_LOAD_IMAGE_UNCHANGED);
    cvDrawContours(img2,*first_contour,CV_RGB(0xff,0x00,0x00),
                   CV_RGB(0xff,0x00,0x00),MAX_LV,1,8,cvPoint(0,0));
    /*IplImage * color_img=cvCreateImage(cvGetSize(img),8,3);
    cvDrawContours(color_img,*first_contour,cvScalarAll(255),
                   cvScalarAll(255),MAX_LV,1,8,cvPoint(0,0));
    for(CvSeq* c=*first_contour;c!=NULL;c=c->h_next)
    {
        if(vdIsCircle(c)>0.0)
            cvEllipseBox(color_img,cvFitEllipse2(c),CV_RGB(0xff,0x00,0x00),1,8,0);
        for(CvSeq* h=c->v_next;h!=NULL;h=h->h_next)
            if(vdIsCircle(h)>0.0)
                cvEllipseBox(color_img,cvFitEllipse2(h),CV_RGB(0xff,0x00,0x00),1,8,0);
    }*/
    /*for(CvSeq* c=*first_contour;c!=NULL;c=c->h_next)
    {
        for(int i=0;i<=c->total;i++)
        {
            CvPoint* point=(CvPoint*)cvGetSeqElem(c,i);
            cvCircle(img_black,*point,0,cvScalarAll(255),1,8,0);
        }
    }*/
    cvShowImage("Contours",img2);
    cvSaveImage("TEST4.bmp",img2,0);
