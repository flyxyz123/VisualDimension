...
///CPPP 笔记: 可能可以用来优化传递指针的函数，例如find_show_contours
/**更深理解了一下指针作为函数参量：
如果要改变指针指向的变量的值，只需要传递指针本身。
如果要使指针指向另一个变量，也就是另一个位置，那么就要改变指针本身的值，这个值是指向变量的位置。此时，需要传递指向此指针的指针！！！
(感觉对深入设计Visual Dimension有用)*/

///最好拆开来find_show_contours使得图片显示不会灰色，更易于操作
///可以用指针或reference(c++)使函数“返回”多个信息，CPPP PP279+
